...
async function seedSampleData() {
  ...
  try {
    ...
    const sampleItems = [
      {
        ...
        web_url: 'https://blueturtle.example',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
      ...
      },
      {
        ...
        telephone: '0980-34-5678',
        web_url: 'https://paradisebeach.example',
        created_at: new Date(),
        updated_at: new Date()
      },
      {
    ...
    for (const item of sampleItems) {
      await prisma.citadela_items.create({
        data: item
      });
    }
    ...
    // 挿入されたデータの確認
    const count = await prisma.citadela_items.count();
